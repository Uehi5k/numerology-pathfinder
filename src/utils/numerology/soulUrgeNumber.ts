 ...
 */
export const getSoulUrgeMeaning = async (num: number, language: LanguageCode = 'en'): Promise<string> => {
  try {
    // Try to load the language-specific meanings
    let soulUrgeMeanings;
    try {
      soulUrgeMeanings = (await import(`../../data/translations/${language}/soulUrgeMeanings.json`)).default;
    } catch (error) {
      // Fallback to English if the translation isn't available
      soulUrgeMeanings = (await import(`../../data/translations/en/soulUrgeMeanings.json`)).default;
    }

    return soulUrgeMeanings[num] || `No meaning available for Soul Urge ${num}`;
  } catch (error) {
    console.error("Error loading soul urge meanings:", error);
    ...
    return meanings[num] || `No meaning available for Soul Urge ${num}`;
  }
};
